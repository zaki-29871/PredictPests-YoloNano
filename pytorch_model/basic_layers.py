...
class SpatialPyramidPooling(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        mid_channel = in_channel//3

        self.branch1 = nn.Sequential(nn.AvgPool2d(kernel_size=2, stride=2),
                                     nn.Conv2d(in_channel, mid_channel, 1, bias=False),
                                     nn.BatchNorm2d(mid_channel),
                                     nn.ReLU(inplace=True))

        self.branch2 = nn.Sequential(nn.AvgPool2d(kernel_size=4, stride=4),
                                     nn.Conv2d(in_channel, mid_channel, 1, bias=False),
                                     nn.BatchNorm2d(mid_channel),
                                     nn.ReLU(inplace=True))

        self.branch3 = nn.Sequential(nn.AvgPool2d(kernel_size=8, stride=8),
                                     nn.Conv2d(in_channel, mid_channel, 1, bias=False),
                                     nn.BatchNorm2d(mid_channel),
                                     nn.ReLU(inplace=True))

        fusion_channel = 2*in_channel
        self.fusion = nn.Sequential(nn.Conv2d(fusion_channel, out_channel, 3, padding=1, bias=False),
    ...
    def forward(self, x):
        ...
        out_branch1 = F.interpolate(self.branch1(x), (H, W), mode='bilinear', align_corners=True)
        out_branch2 = F.interpolate(self.branch2(x), (H, W), mode='bilinear', align_corners=True)
        out_branch3 = F.interpolate(self.branch3(x), (H, W), mode='bilinear', align_corners=True)

        concat = torch.cat((x, out_branch1, out_branch2, out_branch3), 1)
