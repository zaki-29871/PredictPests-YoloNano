...
def get_batch_statistics(outputs, targets, iou_threshold):
    ...
    for sample_i in range(len(outputs)):
        ...
        true_positives_prediction = np.zeros(pred_boxes.shape[0])
        true_positives_annotation = np.zeros(annotations.shape[0])
        ...
        if len(annotations):
            ...
            for pred_i, (pred_box, pred_label) in enumerate(zip(pred_boxes, pred_labels)):
                ...
                if iou >= iou_threshold and pred_label == annotations[box_index, 0] and box_index not in detected_boxes:
                    true_positives_prediction[pred_i] = 1
                    true_positives_annotation[box_index] = 1
                    detected_boxes += [box_index]
        # print('tp ...: ', true_positives, pred_scores, pred_labels)
        batch_metrics.append([true_positives_prediction, true_positives_annotation, pred_scores, pred_labels])
...
def non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.4, use_label_match=True):
    """
    ...
    for image_i, image_pred in enumerate(prediction):
        ...
        while detections.size(0):
            ...
            if use_label_match:
                ...
                # Filter prediction even the class is different
                # Indices of boxes with lower confidence scores, large IOUs and matching labels
...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres):
    ...
    # Output tensors
    obj_mask = torch.full((nB, nA, nG, nG), 0, dtype=torch.bool)
    noobj_mask = torch.full((nB, nA, nG, nG), 1, dtype=torch.bool)
